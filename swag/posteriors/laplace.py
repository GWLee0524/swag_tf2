...
def jacobian(y, x, create_graph=False):
    ...
    for i in range(len(flat_y)):
        ...
        with tf.GradientTape() as tape:
            tape.watch(x)
            grad_x = tape.gradient(flat_y, x, grad_y)
        jac.append(tf.reshape(grad_x, shape=x.shape))
        grad_y[i] = 0.0

    return tf.reshape(tf.stack(jac), shape=y.shape+x.shape)
