...
class BasicBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.relu = tf.keras.layer.ReLU()
        self.batchnorm1 = tf.keras.layers.BatchNormalization()
        self.dropout1 = tf.keras.layers.Dropout(rate=P)
        ...
        self.dropout2 = tf.keras.layers.Dropout(rate=P)
...
class Bottleneck(tf.keras.layers.Layer):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.dropout3 = tf.keras.layers.Dropout(rate=P)
...
class PreResNetDrop(tf.keras.Model):
    def __init__(self, num_classes=10, depth=110, input_shape=None):
        ...
        self.model.add(tf.keras.Dropout(rate=P))
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            #     ),
            # )
            downsample = [tf.keras.layers.Dropout(rate=P), \
                        tf.keras.layers.Conv2D(planes*block.expansion, kernel_size=1, \
...
class PreResNet110Drop:
    ...
    # transform_train = [
    #     lambda img: tf.image.resize(img, 32),
    ...
    #     lambda img: (img-[0.4914, 0.4822, 0.4465])/[0.2023, 0.1994, 0.2010]
    # ]
    # transform_test = [
    #     lambda img: tf.image.resize(img, 32),
    #     lambda img: tf.conver_to_tensor(img),
