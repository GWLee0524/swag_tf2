...
def make_layers(cfg, batch_norm=False):
    ...
    for v in cfg:
        if v == "M":
            ...
            conv2d = tkl.Conv2D(v, kernel_size=3, strides=(1,1), padding='same', kernel_initializer=initializer)
            if batch_norm:
                layers += [
                    tkl.Dropout(rate=P),
                    conv2d,
                ...
                layers += [tkl.Dropout(rate=P), conv2d, tkl.ReLU()]
            in_channels = v
...
class VGGDrop(tf.keras.Model):
    def __init__(self, num_classes=10, depth=16, batch_norm=False, input_shape=None):
        ...
        self.classifier = tf.keras.Sequential([
            tkl.Dropout(rate=P),
            tkl.Dense(512, acivation='relu', kernel_initializer=initializer),
            tkl.Dropout(rate=P),
            tkl.Dense(512, acivation='relu', kernel_initializer=initializer),
            tkl.Dense(num_classes, acivation='softmax', kernel_initializer=initializer)
            ])
