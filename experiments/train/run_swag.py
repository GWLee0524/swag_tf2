...
loaders, num_classes, input_shape = data.loaders(
    args.dataset,
    args.data_path,
    args.batch_size,
    args.num_workers,
...
model = model_cfg.base(*model_cfg.args, num_classes=num_classes, input_shape=input_shape, **model_cfg.kwargs)
...
ckpt_path = os.path.join(args.dir, 'model_ckpts')
manager = tf.train.CheckpointManager(ckpt, ckpt_path, max_to_keep=3)
if args.swa:
    swag_ckpt = tf.train.Checkpoint(step=tf.Variable(1), swag_model=swag_model)
    ckpt_path = os.path.join(args.dir, 'swag_ckpt')
    swag_manager = tf.train.CheckpointManager(swag_ckpt, ckpt_path, max_to_keep=3)
...
if args.resume is not None:
    ...
    start_epoch = ckpt.step.numpy().item()
...
if args.swa and args.swa_resume is not None:
    ...
    swag_ckpt.restore(swag_manager.latest_checkpoint)
    if swag_manager.latest_checkpoint:
        print("Restored from {}".format(swag_manager.latest_checkpoint))
    else:
        print("Initializing from scratch.")
